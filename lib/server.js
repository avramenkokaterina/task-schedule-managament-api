...
class Client {
  ...
  async api() {
    ...
    if (!session && method !== 'signIn' && method !== 'registerUser') {
    ...
    try {
      ...
      if (method === 'signOut') {
        this.application.auth.remove(req, res);
      }
...
class Server {
  constructor(config, application) {
    ...
    const port = process.env.PORT || ports[threadId - 1];
    application.logger.log(process.env.PORT);
    const lib = TRANSPORT[transport];
    const handler = listener(application);
    this.instance = lib.createServer({}, handler);
